...
// Only initialise Resend when a real API key is present.
// This prevents build failures when the env var is missing in production.
 ...
 *
 * Non-fatal: returns { success: true } even when resend is null (no API key).
 * Email delivery is a trust signal — the in-app /pending page is the primary confirmation.
 */
  ...
  try {
    ...
    if (resend) {
      await resend.emails.send({
        ...
        html: buildEmail(name, email),
      })
    } else {
      console.log('[email/apply-confirmation] No RESEND_API_KEY — email skipped (dev mode)')
    }
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #E5E7EB;">
          ...
          <tr>
            <td style="padding:36px 32px 28px;">
              <p style="font-size:26px;font-weight:800;color:#111827;letter-spacing:-0.03em;margin:0 0 8px;">Application received, ${firstName} 👋</p>
              <p style="font-size:15px;color:#374151;line-height:1.6;margin:0 0 24px;">We've received your TradeSource application and will review it personally.</p>
              ...
              <table cellpadding="0" cellspacing="0" style="width:100%;">
                <tr>
                  <td align="center" style="padding-top:8px;">
                    <a href="https://project-bdhbf.vercel.app/jobs" style="display:inline-block;padding:13px 32px;background:#1E40AF;border-radius:10px;font-size:14px;font-weight:700;color:#fff;text-decoration:none;letter-spacing:-0.01em;">Browse Open Jobs →</a>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td style="padding:0 32px 28px;">
              <p style="font-size:12px;color:#9CA3AF;line-height:1.6;margin:0;border-top:1px solid #F3F4F6;padding-top:20px;">
                Questions? Reply to this email or reach us at <a href="mailto:info@tradesource.app" style="color:#1E40AF;text-decoration:none;">info@tradesource.app</a>. TradeSource · Bucks County, PA.
              </p>
            </td>
          </tr>
        </table>
